...
fn sort(highest_calorie: u32, (one, two, three): (u32, u32, u32)) -> (u32, u32, u32) {
    if highest_calorie > one { (highest_calorie, one, two) }
    else if highest_calorie > two { (one, highest_calorie, two) }
    else if highest_calorie > three { (one, two, highest_calorie) }
    else { (one, two, three) }
}
...
pub fn day1() {
    ...
    let (highest, second_highest, third_highest) = file_contents
        .split(SINGLE_ELF_DELIM)
        .fold((0, 0, 0), |acc, elf|  {
            let highest_calorie = elf.split(ITEM_DELIM)
